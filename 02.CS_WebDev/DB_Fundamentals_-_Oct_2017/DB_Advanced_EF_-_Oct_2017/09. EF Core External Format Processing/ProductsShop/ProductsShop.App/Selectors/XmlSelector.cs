    ...
    internal class XmlSelector
    {
        internal static string ExportData()
        {
            using (var db = new ProductsShopDbContext())
            {
                ...
                File.WriteAllText(@"Export\ExportedXml\ProductsInRange.xml", productsInRange);

                string successfullySoldProducts = GetSuccessfullySoldProducts(db);
                File.WriteAllText(@"Export\ExportedXml\SuccessfullySoldProducts.xml", successfullySoldProducts);

                string categoriesByProductsCount = GetCategoriesByProductsCount(db);
                File.WriteAllText(@"Export\ExportedXml\CategoriesByProductsCount.xml", categoriesByProductsCount);

                string usersAndProducts = GetUsersAndProducts(db);
                File.WriteAllText(@"Export\ExportedXml\UsersAndProducts.xml", usersAndProducts);
            }

            return @"Data exported to directory ~Export\ExportedXml";
        }
